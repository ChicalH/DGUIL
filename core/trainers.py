...
class SemanticSTFTrainer(Trainer):
    ...
    def __init__(self,
        ...
        self.T = 0.07
        self.criterion_ent = nn.CrossEntropyLoss()##-log(exp(x_c)/sum(exp(x)))
        self.mse_criterion = nn.MSELoss()
        self.cos_dis = torch.nn.CosineSimilarity(dim=1,eps=1e-5)
        self.use_ema_model = ema_model
        if ema_model:
        ...
        if outputs_1.requires_grad:
            ...
            if self.use_ema_model:
                ...
                for param_q, param_k in zip(self.model.parameters(), self.ema_model.parameters()):
                    param_k.data = param_k.data.clone() * mome_lam + param_q.data.clone() * (1. - mome_lam)
                for buffer_q, buffer_k in zip(self.model.buffers(), self.ema_model.buffers()):
                    buffer_k.data = buffer_q.data.clone()
                with torch.no_grad():
                    outputs_2, feat_2 = self.ema_model(inputs_1,ema_model=True)



                uncertain_mask = outputs_1.var(dim=1).reshape(-1,1)
                loss_2 = torch.mean((torch.exp(-uncertain_mask)) * torch.abs(outputs_1 - outputs_2)+ uncertain_mask)  
                uncertain = torch.mean(uncertain_mask)
                self.summary.add_scalar('uncertain', uncertain.item())

                loss = loss_1 + self.lambda_v2 * loss_2
