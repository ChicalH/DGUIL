...
class SemanticKITTIInternal:
    ...
    def clip_data(self, xyz):
        # pdb.set_trace()
        max_bound = np.asarray([50,50,2])
        min_bound = np.asarray([50,50,4])
    ...
    def return_double_views(self, index):
        ...
        if np.random.rand() < 0.5:
            jiterring = np.random.normal(loc=0., scale=0.01, size=(block_2.shape[0], 3))
        ...
        _, inds_2, inverse_map = sparse_quantize(pc_2_,
                                       return_index=True,
                                       return_inverse=True)
        ratio = np.random.random() * 0.2 + 0.8
        ...
        inverse_map = SparseTensor(inverse_map, pc_2_)
        labels_ = SparseTensor(labels_, pc_2)
