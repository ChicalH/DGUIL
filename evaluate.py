...
def main() -> None:
    ...
    for feed_dict in tqdm(dataflow['test'], desc='eval'):
        ...
        if args.save_pred is not None:
            ...
            if miou>0.355:
                best_save_pred = args.save_pred+'/good/'
                if not os.path.exists(best_save_pred):
                    os.makedirs(best_save_pred)
                outputs_np.tofile(best_save_pred+'/'+filename)

    # trainer.after_epoch()
    miou, ious = mIoU.after_epoch()
    with open('./core/datasets/info.json', 'r') as fp:
        info = json.load(fp)
    name_classes = np.array(info['label'], dtype=str)
    print("===" * 10)
    print(dataset)
    print("iou per class: ", ious)
    for ind_class in range(configs.data.num_classes):
