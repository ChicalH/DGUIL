...
# (Pdb) torch.cuda.is_available() 
# True
# (Pdb) torch.cuda.current_device()
# 0
# (Pdb) torch.cuda.get_device_name(0)
# (Pdb) torch.version.cuda
# '11.6'
...
# RuntimeError: CUDA error: device kernel image is invalid
    ...
    if 'uncertain' in configs.model:
        trainer = SemanticSTFTrainer(model=model,
                                    ...
                                    uncertain = configs.model.uncertain,
                                    lambda_v2 = configs.model.lambda_v2,
                                    ema_model = configs.model.ema_model
                                    )
    else:       
